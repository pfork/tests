...
void nrf24_init(void) {
  ...
  nrf_write_reg(FEATURE, 7);     // Enables the W_TX_PAYLOAD_NOACK + EN_ACK_PAY + DYNAMIC_PAYLOAD command
...
char nrf_send(const unsigned char* address, const unsigned char * tx_buf, const unsigned char tx_buf_len) {
  return nrf_send_ack(address, tx_buf, tx_buf_len, NULL, 0);
}

char nrf_send_ack(const unsigned char* address,
                  const unsigned char * tx_buf, const unsigned char tx_buf_len,
                  unsigned char * rx_buf, const unsigned char rx_buf_len) {
  CE(0);
  ...
  for(i=0;i<max_tx_retries;i++) {
    ...
    if(rx_buf != NULL && rx_buf_len>0 && rx_buf_len<=32) {
      // handle ack
      if(status & RX_DR) {
        unsigned char len;
        ...
        if(len==0) return 0;
        if(len>rx_buf_len) while(1);
        nrf_read_buf(RD_RX_PLOAD, rx_buf, len);// read receive payload from RX_FIFO buffer
        nrf_write_reg(STATUS, RX_DR | TX_DS | MAX_RT);
        return len;
      }
...
void nrf_open_rx(const unsigned char *addr) {
  nrf_open_rx_ack(addr, NULL, 0);
}

void nrf_open_rx_ack(const unsigned char *addr, const unsigned char* ack, const char acklen) {
  CE(0);
  ...
  if(ack!=NULL && acklen >0 && acklen<=32) {
    nrf_cmd_buf(WR_ACK_PLOAD | 0, ack, acklen);
    nrf_cmd_buf(WR_ACK_PLOAD | 0, ack, acklen);
    nrf_cmd_buf(WR_ACK_PLOAD | 0, ack, acklen);
  }
  // power up
...
char nrf_recv(unsigned char * buf, const unsigned char buflen) {
  return nrf_recv_ack(buf, buflen, NULL, 0);
}

char nrf_recv_ack(unsigned char * buf, const unsigned char buflen, const unsigned char* ack, const char acklen) {

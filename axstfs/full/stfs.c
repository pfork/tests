...
int vacuum(Chunk blocks[NBLOCKS][CHUNKS_PER_BLOCK]) {
  ...
  for(b=0;b<NBLOCKS;b++) {
    ...
    } else if((unused[b]+deleted[b])>(candidate_reclaim*9)/10 && random()%2==0) {
...
int stfs_fsck(Chunk blocks[NBLOCKS][CHUNKS_PER_BLOCK]) {
  uint32_t b,c;
  uint32_t clean=1;
  // first pass check out all inodes
  for(b=0;b<NBLOCKS;b++) {
    for(c=0;c<CHUNKS_PER_BLOCK;c++) {
      if(blocks[b][c].type==Inode) {
        const Chunk *chunk;
        uint32_t b1=0, c1=0;
        chunk=find_chunk(blocks, Inode, blocks[b][c].inode.parent, 0,0, &b1, &c1);
        while(chunk && chunk->inode.parent!=1) {
          b1=c1=0;
          chunk=find_chunk(blocks, Inode, chunk->inode.parent, 0,0, &b1, &c1);
        }
        if(!chunk) {
          LOG(1, "[f] null chunk while resolving path\n");
          dump_inode(&chunk->inode);
          //del_chunk(blocks, b, c);
          clean=0;
        }
        if(chunk->inode.type!=0) {
          LOG(1, "[f] invalid path\n");
          dump_inode(&chunk->inode);
          //del_chunk(blocks, b, c);
          clean=0;
        }
        if(chunk->inode.parent!=1) {
          LOG(1, "[f] while resolving path\n");
          dump_inode(&chunk->inode);
          //del_chunk(blocks, b, c);
          clean=0;
        }
  ...
  // second pass clear dead data and garbage chunks
  for(b=0;b<NBLOCKS;b++) {
    for(c=0;c<CHUNKS_PER_BLOCK;c++) {
      if(blocks[b][c].type==Data) {
        const Chunk *chunk;
        uint32_t b1=0,c1=0;
        chunk=find_chunk(blocks, Inode, blocks[b][c].inode.oid, 0,0, &b1, &c1);
        if(!chunk) {
          LOG(1, "[f] dangling chunk oid(%d) b:%d c:%d\n", blocks[b][c].inode.oid, b, c);
          //del_chunk(blocks, b, c);
          clean=0;
        }
      } else if(blocks[b][c].type!=Empty &&
                blocks[b][c].type!=Deleted &&
                blocks[b][c].type!=Inode) {

        LOG(1, "[f] garbage chunk b:%d c:%d\n", b, c);
        //del_chunk(blocks,b,c);
        clean=0;
      }
    }

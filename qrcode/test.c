...
static void print(uint8_t *modules) {
  memset(frame_buffer, 0xff, 1024);
  int y, x;
  for(y=0;y<size;y++) {
    for(x=0;x<size;x++) {
      if(modules[y*size+x]==0) {
        //oled_delpixel((128-33)/2+x, y+(64-33)/2);
        oled_delpixel((128-66)/2+x*2, y*2);
        oled_delpixel((128-66)/2+x*2+1, y*2);
        oled_delpixel((128-66)/2+x*2, y*2+1);
        oled_delpixel((128-66)/2+x*2+1, y*2+1);
      }
    }
  }
  oled_refresh();
...
void test(void) {
  ...
  oled_cmd(0xff);
  while(1) {
  qrcode((uint8_t*) "vioxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx<3", 64);
  }
}

...
const int errorCorrectionLevel_formatBits = 1;
const int size=33;
...
unsigned char functions_bin_lzg[] = {
...
unsigned int functions_bin_lzg_len = 86;

unsigned char modules_bin_lzg[] = {
  
};
unsigned int modules_bin_lzg_len = 147;

void print(uint8_t *modules) {
   int y, x;
   for(y=0;y<size;y++) {
     for(x=0;x<size;x++) {
       if(modules[y*size+x])
...
void drawCodewords(const uint8_t *data, uint8_t *modules, uint8_t *fns) {
...
void setFunctionModule(int x, int y, char isBlack, uint8_t *modules, uint8_t *fns) {
  //modules.at(y).at(x) = isBlack;
  modules[y*33+x]=isBlack;
  //isFunction.at(y).at(x) = true;
  fns[y*33+x]=1;
}

void drawFormatBits(int mask, uint8_t *modules, uint8_t* fns) {
...
void applyMask(int mask, uint8_t *modules, uint8_t *fns) {
  int y, x;
  for(y = 0;y < size;y++) {
    for(x = 0;x < size;x++) {
      char invert;
      ...
      default:  return; // todo fail somehow?
      }
...
int getPenaltyScore(uint8_t *modules) {
  int result = 0;
...
int handleConstructorMasking(uint8_t *modules, uint8_t *fns) {
  int mask;
...
int qrcode(uint8_t *data, int len) {
  // Create the data bit string by concatenating all segments
  const int dataCapacityBits = 640;
  int i;
  uint8_t payload[100];
  payload[0]=(4<<4) | (64 >> 4); // 4(4)64(8)<data>
  payload[1]=(64 << 4) | (data[0] >> 4);
  for(i=0;i+1<len;i++) {
    payload[2+i]=(data[i]<<4) | (data[i+1] >> 4);
  }
...
int main(void) {
  qrcode((uint8_t*) "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx", 64);
  return 0;
}

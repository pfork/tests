...
char _read_otp(uint32_t block, uint32_t byte) {
  return *(char*)(OTP_START_ADDR + block * OTP_BYTES_IN_BLOCK + byte);
}

extern uint8_t firmware_sig[crypto_sign_BYTES];

int _verify_fwsig(void) {
  ...
  int i;
  for(i=0;i<crypto_sign_PUBLICKEYBYTES && _read_otp(0,i)==0xff;i++);
  ...
  if (crypto_sign_verify_detached(firmware_sig, digest, 64, (uint8_t*) (OTP_START_ADDR)) != 0) {
    if (crypto_sign_verify_detached(firmware_sig, digest, 64, (uint8_t*) (OTP_START_ADDR + 1 * OTP_BYTES_IN_BLOCK)) != 0) {
